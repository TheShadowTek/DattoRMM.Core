...
Default (Default)
```
Invoke-RMMApiMethod -Path <String> [-Method <WebRequestMethod>] [-Parameters <Hashtable>] [-Body <Object>]
...
```
Invoke-RMMApiMethod -Path <String> [-Method <WebRequestMethod>] [-Parameters <Hashtable>] [-Body <Object>]
 [-Paginate] -PageElement <String> [-ProgressAction <ActionPreference>] [<CommonParameters>]
```

## DESCRIPTION
The Invoke-RMMApiMethod function is a generic wrapper for calling any Datto RMM API v2
endpoint.
It handles authentication, token refresh, proxy configuration, retry logic, and
...
automatically deserialised into PowerShell objects.
The returned data is not raw JSON -
it is a fully usable PSObject hierarchy.
If you need to inspect the raw JSON payload,
...
https://\<region\>.centrastage.net/api/v2).
You do not need to include the base URL
...
pagination.
The full response object is returned including pageDetails.
...
Invoke-RMMApiMethod -Path 'site' -Method Put -Body $Body
```
...
$Result | Select-Object alertUid, alertMessage
```
...
Do not include the base URL or the /api/v2 prefix.
Only provide the path segment
that follows it.
...
The exact parameters available depend on the endpoint.
Refer to the Datto RMM API
documentation for the supported query parameters on each path.
Common examples:
...
Note: All paginated endpoints accept a 'page' parameter.
The module's typed public
functions always retrieve all pages automatically.
This function allows you to request
a specific page via Parameters if needed (e.g.
@{page = 2}).
...
### -Body
An object or hashtable to send as the JSON request body.
Applies to Post, Put, and
Patch requests.
...
When specified, the function follows the
...
To retrieve a single specific page instead, omit Paginate and
pass the page number via the Parameters parameter (e.g.
@{page = 3}).

```yaml
Type: SwitchParameter
Parameter Sets: Paginate
Aliases:
...
name.
You must specify the correct property so the function can extract the items
from each page.
...
For known endpoints with dedicated functions (e.g.
Get-RMMDevice, Get-RMMSite),
